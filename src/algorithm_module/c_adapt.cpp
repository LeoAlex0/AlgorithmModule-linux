...
AM_PathType PathType_cast( const Algorithm::Graph::PathType &a ) {
    if ( a == Algorithm::Graph::NIL_PATH ) return AM_NIL_PATH;
...
PUBLIC AM_PathType AM_bfs( int idx_start_v, int idx_dest_v,
    ...
    return PathType_cast( Algorithm::Graph::bfs( idx_start_v, idx_dest_v,
...
PUBLIC int AM_bfsStep( int idx_start_v, int idx_dest_v,
                       ...
                       AM_EdgeToAdapter destOf ) {
    return Algorithm::Graph::bfsStep( idx_start_v, idx_dest_v, firstOf, nextOf,
                                      destOf );
}
}
